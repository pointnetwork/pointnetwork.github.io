...
#
[Install]
WantedBy=multi-user.target" > $HOME/pointd.service; sudo mv $HOME/pointd.service /etc/systemd/system/
```

```bash
sudo systemctl enable pointd.service && sudo systemctl daemon-reload
```

### Run pointd

```bash
sytemctl start pointd
```
