...
Check the FAQ for running a validator on Point Chain
...
Point Chain is a public Proof-of-Stake (PoS) blockchain, meaning that validator's weight is determined by the amount of staking tokens (POINT) bonded as collateral. These staking tokens can be staked directly by the validator or delegated to them by POINT holders.
...
- **Validator's Address**: `pointvaloper1-` address. This is the address used to identify your validator publicly. The private key associated with this address is used to bond, unbond, and claim rewards.
...
- **Minimum self-bond amount**: Minimum amount of POINT the validator needs to have bonded at all times. If the validator's self-bonded stake falls below this limit, its entire staking pool will be unbonded.
- **Initial self-bond amount**: Initial amount of POINT the validator wants to self-bond.

```bash
pointd tx staking create-validator
    --pubkey pointvalconspub1zcjduepqs5s0vddx5m65h5ntjzwd0x8g3245rgrytpds4ds7vdtlwx06mcesmnkzly
    --amount "2apoint"
    --from tmp
    ...
    --moniker "validator"
    --chain-id "point_9000-4"
    --gas auto
    --node tcp://127.0.0.1:26647
```

:::danger
🚨 **DANGER**: <u>Never</u> create your mainnet validator keys using a [`test`](./../users/keys/keyring) keying backend. Doing so might result in a loss of funds by making your funds remotely accessible via the `eth_sendTransaction` JSON-RPC endpoint.

Ref: [Security Advisory: Insecurely configured geth can make funds remotely accessible](https://blog.ethereum.org/2015/08/29/security-alert-insecurely-configured-geth-can-make-funds-remotely-accessible/)
:::

Once a validator is created and registered, POINT holders can delegate POINT to it, effectively adding stake to its pool. The total stake of a validator is the sum of the POINT self-bonded by the validator's operator and the POINT bonded by external delegators.
...
- **Tendermint Key**: This is a unique key used to sign block hashes. It is associated with a public key `pointvalconspub`.
    - Generated when the node is created with `pointd init`.
    - Get this value with `pointd tendermint show-validator`
    e.g. `pointvalconspub1zcjduc3qcyj09qc03elte23zwshdx92jm6ce88fgc90rtqhjx8v0608qh5ssp0w94c`
...
:::warning
A validator's operator key is directly tied to an application key, but uses reserved prefixes solely for this purpose: `pointvaloper` and `pointvaloperpub`
:::
...
:::warning
Delegations are not necessarily bonded. POINT can be delegated and bonded, delegated and unbonding, delegated and unbonded, or liquid.
:::

### What is "self-bond"? How can I increase my "self-bond"?

The validator operator's "self-bond" refers to the amount of POINT stake delegated to itself. You can increase your self-bond by delegating more POINT to your validator account.

### Is there a testnet faucet?

<!-- markdown-link-check-disable-next-line -->
If you want to obtain coins for the testnet, you can do so by using the [faucet](https://faucet.point.dev/).

### Is there a minimum amount of POINT that must be staked to be an active (bonded) validator?
...
- **Amount of self-bonded POINT:** Number of POINT a validator self-bonded to its staking pool. A validator with higher amount of self-bonded POINT has more skin in the game, making it more liable for its actions.

- **Amount of delegated POINT:** Total number of POINT delegated to a validator. A high stake shows that the community trusts this validator, but it also means that this validator is a bigger target for hackers. Validators are expected to become less and less attractive as their amount of delegated POINT grows. Bigger validators also increase the centralization of the network.
...
Staking POINT can be thought of as a safety deposit on validation activities. When a validator or a delegator wants to retrieve part or all of their deposit, they send an unbonding transaction. Then, POINT undergo a *three weeks unbonding period* during which they are liable to being slashed for potential misbehaviors committed by the validator before the unbonding process started.

Validators, and by association delegators, receive block provisions, block rewards, and fee rewards. If a validator misbehaves, a certain portion of its total stake is slashed (the severity of the penalty depends on the type of misbehavior). This means that every user that bonded POINT to this validator gets penalized in proportion to its stake. Delegators are therefore incentivized to delegate to validators that they anticipate will function safely.

### Can a validator run away with its delegators' POINT?

By delegating to a validator, a user delegates staking power. The more staking power a validator has, the more weight it has in the consensus and processes. This does not mean that the validator has custody of its delegators' POINT. *By no means can a validator run away with its delegator's funds*.
...
### How often will a validator be chosen to propose the next block? Does it go up with the quantity of POINT staked?

The validator that is selected to mine the next block is called the **proposer**, the "leader" in the consensus for the round. Each proposer is selected deterministically, and the frequency of being chosen is equal to the relative total stake (where total stake = self-bonded stake + delegators stake) of the validator. For example, if the total bonded stake across all validators is 100 POINT, and a validator's total stake is 10 POINT, then this validator will be chosen 10% of the time as the proposer.
...
- **Block rewards:** Native tokens of applications run by validators (e.g. POINT on Point Chain) are inflated to produce block provisions. These provisions exist to incentivize POINT holders to bond their stake, as non-bonded POINT will be diluted over time.
- **Transaction fees:** Point Chain maintains a whitelist of token that are accepted as fee payment. The initial fee token is the `point`.
...
Block provisions (rewards) are distributed proportionally to all validators relative to their total stake (voting power). This means that even though each validator gains POINT with each provision, all validators will still maintain equal weight.
...
Let's take a concrete example to illustrate the aforementioned concept. In this example, there are 10 validators with equal stake. Each of them applies a 1% commission and has 20% of self-bonded POINT. Now comes a successful block that collects a total of 1005 POINT in fees. Let's assume that the proposer included 100% of the signatures in its block. It thus obtains the full bonus of 5%.
...
### Do validators need to self-bond POINT
...
Even though there is no obligation for validators to self-bond POINT, delegators should want their validator to have self-bonded POINT in their staking pool. In other words, validators should have skin-in-the-game.

In order for delegators to have some guarantee about how much skin-in-the-game their validator has, the latter can signal a minimum amount of self-bonded POINT. If a validator's self-bond goes below the limit that it predefined, this validator and all of its delegators will unbond.
