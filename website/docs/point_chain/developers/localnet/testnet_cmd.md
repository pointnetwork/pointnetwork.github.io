...
The `pointd testnet` subcommand makes it easy to initialize and start a simulated test network for testing purposes.

In addition to the commands for [running a node](../../validators/quickstart/run_node), the `pointd` binary also includes a `testnet` command that allows you to start a simulated test network in-process or to initialize files for a simulated test network that runs in a separate process.
...
A node directory is created for each validator node. Within each node directory is a `pointd` directory. The `pointd` directory is the home directory for each node, which includes the configuration and data files for that node (i.e. the same files included in the default `~/.pointd` directory when running a single node).
...
preparing test network with chain-id "point_1276974-1"
...
```bash
pointd keys add test --recover
```

Check the balance of the account address:

```bash
pointd q bank balances [address]
```
