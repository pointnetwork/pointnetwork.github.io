...
This command creates a 4-node network using the `pointdnode` Docker image.
...
calling the `pointd testnet` command. This outputs a handful of files in the
`./build` directory:
...
Each `./build/nodeN` directory is mounted to the `/pointd` directory in each container.
...
To interact with `pointd` and start querying state or creating txs, you use the
`pointd` directory of any given node as your `home`, for example:

```bash
pointd keys list --home ./build/node0/pointd
```
...
:::tip
**Note**: Each node's seed is located at `./build/nodeN/pointd/key_seed.json` and can be restored to the CLI using the `pointd keys add --restore` command
:::
