...
Learn how to deploy a simple Solidity-based smart contract to Point Chain using the Truffle environment
...
This will allow your contract to connect to your Point Chain local node.
...
You should see incoming deployment logs in the Point Chain daemon Terminal tab for each transaction (one to deploy `Migrations.sol` and the other to deploy `Counter.sol`).
...
Now, you can run the Truffle tests using the Point Chain node using the `test` command:
