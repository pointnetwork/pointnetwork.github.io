...
looking for. For example, a an Ethereum transaction on Point Chain (`MsgEthereumTx`) triggers an `event` of type `ethermint` and
...
Point Chain also supports the Ethereum [JSON-RPC](./server.md) filters calls to
...
```bash
pointd start --rpc.laddr="tcp://127.0.0.1:26657"
```
...
Since Point Chain runs uses Tendermint Core as it's consensus Engine and it's built with the Cosmos
SDK framework, it inherits the event format from them. However, in order to support the native Web3
compatibility for websockets of the [Ethereum's PubSubAPI](https://geth.ethereum.org/docs/rpc/pubsub), Point Chain needs to cast the Tendermint
...
```bash
pointd start  --json-rpc.address"0.0.0.0:8545" --json-rpc.ws-address="0.0.0.0:8546" --evm.rpc.api="eth,web3,net,txpool,debug" --json-rpc.enable
```
