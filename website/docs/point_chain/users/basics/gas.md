...
For this same reason, in Point Chain it is possible to send transactions with `0` fees for transaction
...
(i.e `ValidateBasic`) function as well as on the custom `AnteHandler` defined by Point Chain.
